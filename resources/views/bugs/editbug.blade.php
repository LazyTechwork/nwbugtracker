    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                ...
                <form action="{{ route('bugs.editbug', ['id'=>$bug->id]) }}" method="POST">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="bugname" placeholder="Коротко о баге" name="name" value="{{ old('name', $bug->name) }}"  max="100" required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <select name="priority" id="bugpriority" class="form-control">
                            @foreach(App\Bug::$priorities as $type)
                                <option value="{{ $loop->index }}" @if($loop->index == 1 || $loop->index == old('priority', $bug->priority)) selected @endif>{{ $type }}</option>
                            @endforeach
                        </select>
                    </div>
